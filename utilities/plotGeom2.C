...
//    gStyle->SetOptStat(0);
    ...
    gtree->Print();
    gtree->SetAlias("yy","(xtals_ylab[0]<0)*(xtals_ylab[0]-7.061)+(xtals_ylab[0]>=0)*(xtals_ylab[0]+12.37)");
//    gtree->SetAlias("yy","xtals_ylab");
    gtree->SetAlias("xx","xtals_xlab[0]");
    gtree->SetAlias("zz","xtals_zlab[0]");
    ...
    for(int i=0;i<limit;i++) {
        gtree->Draw(Form("phi:theta>>h%d(360.,-180.,180.,180.,0.,180.)",i),Form("xtals_quadNum[0]==%d",i+1),"goff");
