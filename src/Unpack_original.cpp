...
void Unpack::CombineReader(fileListStruct run) {
    ...
    for(size_t i = 0; i < orrubaTimeStamps_.size(); i++) {
        ...
        Long64_t gretinaTime_sdp = 0;
        for(size_t j = 0; j < gretinaTimeStamps_.size(); j++) {
//             std::cout << gretinaTime_sdp;

	    if((gretinaTimeStamps_[j].second < gretinaTime_sdp)) {
	    std::cout << gretinaTime_sdp;
	    } 
	    if((gretinaTimeStamps_[j].second > gretinaTime_sdp)) {
	    gretinaTime_sdp = gretinaTimeStamps_[j].second;
	    }
	    
            size_t timeDiff = fabs(orrubaTime - gretinaTimeStamps_[j].second);
//            size_t timeDiff = orrubaTime - gretinaTimeStamps_[j].second;

            if((timeDiff < 200) && (timeDiff < closestTime)) {    
//            if((timeDiff < 100) && (timeDiff > -10) ) {    
            ...
            if(timeDiff > 50000 && (gretinaTimeStamps_[j].second > orrubaTime)) break; // SDP modded 2021/07/29 to add 500 to look backward in time-difference, and to increase from 50000 to 500000
        }
        ...
        if((i % 1000 == 0) && found) gretinaTimeStamps_.erase(gretinaTimeStamps_.begin(), gretinaTimeStamps_.begin() + found_index); //500000000
	
    }  //close of CombineReader
